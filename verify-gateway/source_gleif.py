...
def gleif_verify(
    ...
    # Name search — try legalName then fulltext. For each strategy, also try
    # a corporate-suffix-stripped variant since GLEIF stores legalName with
    # full punctuation (e.g. "Emirates NBD Bank P.J.S.C.") while Onboarding
    # typically supplies the trading-name form (e.g. "EMIRATES NBD BANK PJSC").
    # Stripping the suffix lets fulltext find them.
    def _strip_corp_suffix(name: str) -> str:
        # Strip common corporate-form suffixes (case-insensitive, with/without
        # periods/dots/spaces). Run from longest to shortest so e.g. "P.J.S.C."
        # is matched before "P.J.S.".
        suffixes = [
            r"P\.?\s?J\.?\s?S\.?\s?C\.?",   # AE/middle east PJSC
            r"P\.?\s?S\.?\s?C\.?",            # AE PSC
            r"L\.?\s?L\.?\s?C\.?",            # AE LLC
            r"F\.?\s?Z\.?\s?C\.?\s?O\.?",    # AE Free Zone Co
            r"F\.?\s?Z\.?\s?E\.?",            # AE Free Zone Establishment
            r"S\.?\s?A\.?\s?E\.?",            # EG Shareholding co.
            r"A\.?\s?O\.?\s?Y\.?",            # TR AS variants
            r"A\.?\s?S\.?",                     # TR Anonim Sirketi
            r"L\.?\s?T\.?\s?D\.?",            # generic Ltd
            r"S\.?\s?A\.?",                     # generic SA
            r"P\.?\s?L\.?\s?C\.?",            # generic PLC
            r"GmbH",
            r"BV", r"NV",
        ]
        out = name
        for sx in suffixes:
            out = re.sub(rf"\b{sx}\s*$", "", out, flags=re.IGNORECASE).strip().rstrip(",.")
        return out.strip() or name

    name_variants = [entity_name]
    stripped = _strip_corp_suffix(entity_name)
    if stripped != entity_name:
        name_variants.append(stripped)

    for variant in name_variants:
        for mode in ("name_legalName", "name_fulltext"):
            r = eng.run(_config_for(cc, mode), variant, {})
            if r.get("verified"):
                return r
