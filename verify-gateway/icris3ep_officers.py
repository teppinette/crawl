    ...
    try:
        with sync_playwright() as pw:
            ...
            try:
                ...
                # CR Number
                cr_m = re.search(r"(?:CR\s*(?:No|Number)|Company\s*Number)\s*[:.]?\s*(\d{4,10})", body)
                if cr_m:
                    result["cr_number"] = cr_m.group(1).strip()

                # Date of Incorporation / Registration (HK Companies Registry uses
                # DD/MM/YYYY or DD-MM-YYYY)
                for pat in (r"Date of (?:Incorporation|Registration)\s*[:.]?\s*(\d{1,2}[/-]\d{1,2}[/-]\d{4})",
                            r"Date of (?:Incorporation|Registration)\s*[:.]?\s*(\d{4}-\d{2}-\d{2})",
                            r"Incorporated\s*(?:on)?\s*[:.]?\s*(\d{1,2}\s+[A-Z][a-z]+\s+\d{4})"):
                    dm = re.search(pat, body)
                    if dm:
                        result["incorporation_date"] = dm.group(1).strip()
                        break

                # Company Status (HK: Live, Dissolved, Struck Off, Winding Up, Removed)
                st_m = re.search(r"(?:Company\s*Status|Status)\s*[:.]?\s*(Live|Dissolved|Struck\s*Off|Winding\s*Up|Removed|Active)", body, re.IGNORECASE)
                if st_m:
                    result["status"] = st_m.group(1).strip()

                # Company Type (Private/Public/Limited by Guarantee/Non-HK)
                ct_m = re.search(r"Company\s*Type\s*[:.]?\s*([A-Z][A-Za-z \-]+(?:Company|Limited|Liability|Guarantee))", body)
                if ct_m:
                    result["company_type"] = ct_m.group(1).strip()

                # Date of Dissolution if struck off
                dd_m = re.search(r"Date of (?:Dissolution|Removal|Strike[\s\-]?Off)\s*[:.]?\s*(\d{1,2}[/-]\d{1,2}[/-]\d{4})", body)
                if dd_m:
                    result["dissolution_date"] = dd_m.group(1).strip()
            finally:
                try:
